...
public class SergioReis extends TeamRobot {
	...
	private void crearBaseConocimiento() {
		...
		String ficheroReglas = SergioReis.RULES_FILE;

		DEBUG.mensaje("crear base de conocimientos");
		kbuilder = KnowledgeBuilderFactory.newKnowledgeBuilder();

		DEBUG.mensaje("cargar reglas desde "+ficheroReglas);
		kbuilder.add(ResourceFactory.newClassPathResource(ficheroReglas, SergioReis.class), ResourceType.DRL);
	...
	private void cargarEstadoRobot() {
		...
		schedules.add(ksession.insert(estadoRobot));
	}
	...
	private void cargarEstadoBatalla() {
		EstadoBatalla estadoBatalla =
			new EstadoBatalla(getBattleFieldWidth(), getBattleFieldHeight(),
					...
					getTime(),
					getOthers());
		schedules.add(ksession.insert(estadoBatalla));
	}

	private void limpiarHechosIteracionAnterior() {
		for (FactHandle referenciaHecho : this.schedules) {
	...
	private Vector<Accion> recuperarAcciones() {
		...
		for (QueryResultsRow resultado : ksession.getQueryResults(SergioReis.CONSULTA_ACCIONES)) {
	...
	public void onBulletHit(BulletHitEvent event) {
		...
		if(isTeammate(hitRobot)){
			System.out.println("Atingi um amigo!");
		}
		schedules.add(ksession.insert(event));
	}
	...
	public void onScannedRobot(ScannedRobotEvent ee) {
		...
		if(isTeammate(name)){
			System.out.println("Robo da mesma esp�cie!");
		} else {
	...
	public void onMessageReceived(MessageEvent event) {
		if(event.getMessage() instanceof EnemySpotted){
			schedules.add(ksession.insert(event.getMessage()));
		}
	}
